...
def entropy(F1, feat, lamda, eta=1.0):
    ## loss not (reversal) to train Feature extraction backbone
    out_t1 = F1(feat, reverse=True, eta=-eta)
    out_t1 = F.softmax(out_t1)
    ## loss to train Classifier
    ## note that here the loss is standard entropy
...
def adentropy(F1, feat, lamda, eta=1.0):
    ## loss reversal to train Feature extraction backbone
    out_t1 = F1(feat, reverse=True, eta=eta)
    out_t1 = F.softmax(out_t1)
    ## loss to train Classifier
    ## note that here the loss is negative entropy
